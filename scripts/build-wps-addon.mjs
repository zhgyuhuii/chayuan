...
function writeInstallReadme(pkg, releaseDir) {
	...
	const lines = [
		...
		'',
		'一键安装：',
		'- Windows：在项目根目录执行 npm run build:wps-exe，得到自解压 .exe，双击即可写入 %AppData%\\Kingsoft\\wps\\jsaddons。',
		'- macOS：执行 bash scripts/build-macos-pkg.sh，双击 release 下的 .pkg（未签名时可能需右键 → 打开）。',
		'- Linux（Debian/Ubuntu）：执行 bash scripts/build-linux-deb.sh，sudo dpkg -i release/*.deb。',
		'',
