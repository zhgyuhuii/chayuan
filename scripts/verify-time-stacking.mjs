...
ok('time 叠加(30+30=60天)', Math.abs(new Date(r1.timeExpireAt).getTime() - (now + 60 * DAY)) < 2000, `exp=${r1.timeExpireAt}`)

// time 从过期/free(base=now)
const r2 = computeStackedRec({ plan: 'free' }, { kind: 0, value: 30 }, 'S2', now)
ok('time 从now(30天)', Math.abs(new Date(r2.timeExpireAt).getTime() - (now + 30 * DAY)) < 2000, `exp=${r2.timeExpireAt}`)

// count 叠加
const e3 = { plan: 'active', kind: 'count', value: 50 }
const r3 = computeStackedRec(e3, { kind: 1, value: 50 }, 'S3', now)
ok('count 叠加(50+50=100)', r3.counts === 100, `counts=${r3.counts}`)
...
try {
  ...
  ok('损坏expireAt兜底(now+30天)', Math.abs(new Date(rc.timeExpireAt).getTime() - (now + 30 * DAY)) < 2000, `exp=${rc.timeExpireAt}`)
} catch (e) { threw = true }
