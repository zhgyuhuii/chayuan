...
对接 Claude Code / Codex / Cursor / Hermes / OpenClaw 后，可直接对**当前 WPS 文档**使用下列提示词（可复制改写）。完整分类表见仓库根目录 [README.md「常用使用场景与提示词」](../README.md)。

#### 错别字 / 标点 / 正确性

- 帮我检查文档中的错别字，用批注标出原文和建议改法  
- 重点检查表格单元格里的错别字，批注必须钉在具体错字上  
- 帮我检查标点符号是否正确（中英文标点、引号配对），用批注标出  
- 帮我核对这篇文章的正确性：错别字、标点、语法、前后矛盾一并查  
- 先 dryRun 汇总问题，不要改正文；我确认后再写成批注  
- 找出错别字并直接改正正文（先预览清单，确认后再执行）  
...
- 检查合同是否缺主体、金额、期限、违约责任，列风险批注  

#### 翻译 / 润色 / 写回

- 帮我翻译成英文，并把译文插到每一段后面  
- 在不改变原意的前提下润色全文，改动处用批注说明  
- 生成 300 字以内摘要，插入文档开头（先预览）  
- 把「永鹅」全部替换为「咏鹅」，先预览再确认  
- 在每一段后面插入译文/标记（先预览第一段）  

#### 保密 / 多文档 / 知识库

- 帮我检查这份材料的保密风险与敏感信息，用批注标出  
- 查找疑似身份证号、手机号并批注  
- 打开指定路径文档并确保其为当前活动文档  
- 对照知识库制度检查是否违规表述，引用条文批注  
